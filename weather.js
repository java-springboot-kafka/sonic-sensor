...
module.exports = function (RED) {
    ...
    function weatherNode(config) {
        ...
        node.on('input', function (msg) {
            ...
            if (!locationName || !mapbox_key || !weatherstack_key) {
                ...
                geocode(locationName, mapbox_key, (error, {
                    ...
                    if (error) {
                        ...
                        forecast(latitude, longitude, weatherstack_key, (error, forecastData) => {
                            if (error) {
                                ...
                                msg.payload = {
                                    place: location,
                                    weather: forecastData.weatherDescription,
                                    actualTemperature: forecastData.actualTemp,
                                    feelsLikeTemperature: forecastData.feelsLikeTemp
                                }
                                node.send(msg);
                            }
                        });
    ...
    RED.nodes.registerType("weather-data", weatherNode, {
        credentials: {
            mapbox_key: {type: "password"}, weatherstack_key: {type: "password"}
        }
    });
}